...
from model_for_wormnd import NIT_Registration, neuron_data_pytorch
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrain_path", default='../model/model.bin', type=str)
    tic = time.time()
    ...
    print(args.data_mode+'_train', args.data_mode+'_eval')
    train_data = neuron_data_pytorch(args.train_path, batch_sz=args.batch_size, shuffle=True, rotate=True, mode=args.data_mode+'_train')
    dev_data = neuron_data_pytorch(args.eval_path, batch_sz=args.batch_size, shuffle=False, rotate=True, mode=args.data_mode+'_eval')
    ...
    if args.use_pretrain:
        path_to = os.path.join(PATH_MODEL,'pretrain')
        pretrain_path = os.path.join(path_to, 'model.bin')
    ...
    print(os.getcwd())
    print(os.path.join(os.getcwd(), args.model_path))
    # assert 0==1
    ...
    while True:
        ...
        for batch_idx, data_batch in enumerate(train_data_loader):
            ...
            if train_iter % args.valid_niter == 0:
                ...
                model_name_1 = '{}_nh{}_nl{}_ft{}_data{}_elam_{}_model{}_epoch{}.bin'.format(args.model_name, args.n_hidden,
                                                                              args.n_layer,
                                                                              args.f_trans, args.data_mode,
                                                                              args.lamb_entropy,
                                                                              args.model_idx, epoch)
                # os.mkdir(args.model_path)
                model_save_path_1 = os.path.join(args.model_path, str(args.model_idx))
                try:
                    print(model_save_path_1)
                    os.mkdir(model_save_path_1)
                except:
                    print('exist')
                model_save_path_1 = os.path.join(args.model_path, str(args.model_idx), model_name_1)
                # model_save_path_1 = os.path.join(model_save_path,str(epoch))
                print(model_save_path_1)
                model.save(model_save_path_1)
                torch.save(optimizer.state_dict(), model_save_path_1 + '.optim')

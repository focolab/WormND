...
class neuron_data_pytorch(Dataset):
    ...
    def load_path(self, path, batch_sz):
        ...
        print(self.folders[:10])
        import pandas as pd
        name = r"C:\Users\jd\Desktop\tr_te_val_splits.csv"
        name = "/scratch/jd4587/fDNC_Daniel/tr_te_val_splits.csv"
        name = "Daniel/group_assigns.csv"
        name = "group_assigns.csv"
        try:
            group_assigns_df = pd.read_csv(name)
        except:  
            name2 = os.path.join('wormnd', name)
            group_assigns_df = pd.read_csv(name2)

        group_original = group_assigns_df[group_assigns_df['Group'] == 'original']['Filename'].tolist()
        group_0 = group_assigns_df[group_assigns_df['Group'] == '0']['Filename'].tolist()
        group_1 = group_assigns_df[group_assigns_df['Group'] == '1']['Filename'].tolist()
        group_2 = group_assigns_df[group_assigns_df['Group'] == '2']['Filename'].tolist()
        group_3 = group_assigns_df[group_assigns_df['Group'] == '3']['Filename'].tolist()
        group_4 = group_assigns_df[group_assigns_df['Group'] == '4']['Filename'].tolist()
        print(len(group_original))
        print(len(group_0))
        print(len(group_1))
        print(len(group_2))
        print(len(group_3))
        ...
        # self.mode = 'train_0'
        if self.mode.startswith('train_0'):
            train_set_fold_id = group_original + group_1 + group_2 + group_3 + group_4
            val_set_fold_id = group_0
        elif self.mode.startswith('train_1'):
            train_set_fold_id = group_original + group_0 + group_2 + group_3 + group_4
            val_set_fold_id = group_1
        elif self.mode.startswith('train_2'):
            train_set_fold_id = group_original + group_0 + group_1 + group_3 + group_4
            val_set_fold_id = group_2
        elif self.mode.startswith('train_3'):
            train_set_fold_id = group_original + group_0 + group_1 + group_2 + group_4
            val_set_fold_id = group_3
        elif self.mode.startswith('train_4'):
            train_set_fold_id = group_original + group_0 + group_1 + group_2 + group_3
            val_set_fold_id = group_4


        group_all = group_assigns_df['Filename'].tolist()
        group_wo_HL = group_assigns_df[group_assigns_df['Dataset'] != 'HL']['Filename'].tolist()
        group_wo_EY = group_assigns_df[group_assigns_df['Dataset'] != 'EY']['Filename'].tolist()
        group_wo_SK2 = group_assigns_df[group_assigns_df['Dataset'] != 'SK2']['Filename'].tolist()
        group_wo_SK1 = group_assigns_df[group_assigns_df['Dataset'] != 'SK1']['Filename'].tolist()
        group_wo_KK = group_assigns_df[group_assigns_df['Dataset'] != 'KK']['Filename'].tolist()
        group_wo_SF = group_assigns_df[group_assigns_df['Dataset'] != 'SF']['Filename'].tolist()

        temp = group_assigns_df[group_assigns_df['Dataset'] != 'SK1']
        temp = temp[temp['Dataset'] != 'SK2']
        group_wo_SK = temp['Filename'].tolist()

        print('group_wo_HL:', len(group_wo_HL))
        print('group_wo_EY:', len(group_wo_EY))
        print('group_wo_SK2:', len(group_wo_SK2))
        print('group_wo_SK1:', len(group_wo_SK1))
        print('group_wo_KK:', len(group_wo_KK))
        print('group_wo_SF:', len(group_wo_SF))

        if self.mode.startswith('train_wo_HL'):
            train_set_fold_id = group_wo_HL
            val_set_fold_id = list(set(group_all)- set(train_set_fold_id))
        elif self.mode.startswith('train_wo_EY'):
            train_set_fold_id = group_wo_EY
            val_set_fold_id = list(set(group_all)- set(train_set_fold_id))
        elif self.mode.startswith('train_wo_SK2'):
            train_set_fold_id = group_wo_SK2
            val_set_fold_id = list(set(group_all)- set(train_set_fold_id))
        elif self.mode.startswith('train_wo_SK1'):
            train_set_fold_id = group_wo_SK1
            val_set_fold_id = list(set(group_all)- set(train_set_fold_id))
        elif self.mode.startswith('train_wo_KK'):
            train_set_fold_id = group_wo_KK
            val_set_fold_id = list(set(group_all)- set(train_set_fold_id))
        elif self.mode.startswith('train_wo_SF'):
            train_set_fold_id = group_wo_SF
            val_set_fold_id = list(set(group_all) - set(train_set_fold_id))

        elif self.mode.startswith('train_wo_SK'):
            train_set_fold_id = group_wo_SK
            val_set_fold_id = list(set(group_all) - set(train_set_fold_id))
 
        # print(self.folders)
        if self.mode.endswith('_train'):
            print(self.folders[0].split('\\')[-2] )
            print(train_set_fold_id[:10])
            self.folders = [fold for fold in self.folders if fold.split('\\')[-2] in train_set_fold_id]
        if self.mode.endswith('_eval'):
            print(val_set_fold_id[:10])
            self.folders = [fold for fold in self.folders if fold.split('\\')[-2] in val_set_fold_id]
